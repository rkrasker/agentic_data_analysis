...
Added resolver generation usability updates:
13. **Configurable component_id column** for resolver generation (supports branch-based validation).
14. **Function-only notebooks** for synthetic rebuild and resolver generation workflows.
...
| `src/strategies/resolver/generator/generate.py` | Added component_id column override for validation data |
| `src/preprocessing/difficulty/loader.py` | Fixed default project root resolution |
| `rebuild_synthetic_and_preprocessing.ipynb` | Function-only rebuild notebook (lazy imports) |
| `resolver_generation.ipynb` | Function-only resolver generation notebook (lazy imports) |
