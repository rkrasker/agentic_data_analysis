...
**Status:** Regex extraction + synthetic adapter updated for v4.1; component routing pending; ADR-010 schema refactoring pending
...
> **Note:** ADR-010 specifies schema refactoring. After ADR-010 implementation:
> - Input `raw.parquet` will contain only: `source_id`, `soldier_id`, `raw_text`
> - Synthetic metadata (clerk_id, quality_tier, state_id linkage) will be in separate `synthetic_records.parquet`
> - Difficulty outputs will use `inferred_` prefix in `inferred_difficulty.parquet`

**Input (current synthetic v4.1 raw.parquet):**
- `source_id`, `soldier_id`, `state_id`, `raw_text`
- `clerk_id`, `situation_id`, `quality_tier`
- `path_completeness`, `levels_provided`, `extraction_signals` (synthetic-only)

**Output:**
- `data/synthetic/canonical.parquet` (record-level)
  - core columns: `source_id`, `soldier_id`, `state_id`, `raw_text`
  - extraction columns from regex engine
- `data/synthetic/synthetic_metadata.parquet`
  - join key: `(source_id, soldier_id, state_id)`
  - synthetic-only fields + any unexpected input columns
- `data/synthetic/inferred_difficulty.parquet` (pending ADR-010)
...
**Routing:**
- `state_id` → canonical
- `path_completeness`, `levels_provided`, `extraction_signals` → metadata
- unexpected columns → metadata (with warning)
